...
let AppComponent = class AppComponent {
    constructor(formBuilder, dataService, translate) {
    ...
    ngOnInit() {
        ...
        this.dataService.GetIngredients().then((data) => {
    ...
    onSubmit() {
        ...
        this.dataService.Convert(ingredientSelection, inputUnit, outputUnit, inputUnitValue).then((response) => {
            // this.htmlVariable = response.data.result + " " + (v => v.id == this.contactForm.get('name').value)[0].name;
            console.log('selected name', this.units.filter(v => v.name == outputUnit));
            this.unitText = this.units.filter(v => v.id == outputUnit)[0].name;
            this.htmlVariable = response.data.result + " " + this.unitText;
        });
    }
};
...
let DataService = class DataService {
    ...
    GetIngredients(language = "en", apiVersion = "1.0") {
        return this.client.ingredients(language, apiVersion);
    }
    /**
     * GetLanguages
     */
    GetLanguages(apiVersion = "1.0") {
        return this.client.languages(apiVersion);
    }
    /**
     * Convert
     */
    Convert(ingredientId, from, to, quantity, language = "en", apiVersion = "1.0") {
...
(function (ApiClient) {
    class Client {
         ...
         * @return Success
         */
        conversions(ingredientId, from, to, quantity, language, api_version) {
            let url_ = this.baseUrl + "/api/Conversions?";
         ...
         * @return Success
         */
        ingredients(language, api_version) {
         ...
         * @return Success
         */
        ingredients2(id, language, api_version) {
    ...
    class ConversionDTOErrorDTOResponseDTO {
        constructor(data) {
            if (data) {
                for (var property in data) {
                    if (data.hasOwnProperty(property))
    ...
    class IngredientDTOListErrorDTOResponseDTO {
        constructor(data) {
            if (data) {
                for (var property in data) {
                    if (data.hasOwnProperty(property))
    ...
    class IngredientDTOErrorDTOResponseDTO {
        constructor(data) {
            if (data) {
                for (var property in data) {
                    if (data.hasOwnProperty(property))
    ...
    class LanguageDTOListErrorDTOResponseDTO {
        constructor(data) {
            if (data) {
                for (var property in data) {
                    if (data.hasOwnProperty(property))
    ...
    class LanguageDTOErrorDTOResponseDTO {
        constructor(data) {
            if (data) {
                for (var property in data) {
                    if (data.hasOwnProperty(property))
