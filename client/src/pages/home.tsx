...
import type { Ingredient, ConversionResult, Conversion } from "@shared/schema";
...
export default function Home() {
  ...
  const { data: history = [] } = useQuery<Conversion[]>({
    queryKey: ["/api/conversions"],
  });

  // Convert mutation
  const convertMutation = useMutation({
    mutationFn: async (data: { ingredient: string; amount: number; fromUnit: string }) => {
  ...
  const clearHistoryMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("DELETE", "/api/conversions", {});
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/conversions"] });
  ...
  const quickConversions = [
    { text: "1 cup flour", detail: "≈ 125g" },
    { text: "1 cup sugar", detail: "≈ 200g" },
    { text: "1 tbsp butter", detail: "≈ 14g" },
    { text: "1 tsp salt", detail: "≈ 6g" },
  ];
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                ...
                {conversionResult && (
                  <div className="mt-4 text-xs text-gray-500 flex items-center">
                    ...
                    Based on {conversionResult.ingredient} density
                  </div>
                )}
              </div>
            </div>
      ...
      <footer className="bg-white border-t border-gray-200 mt-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <h4 className="font-semibold text-gray-900 mb-3">About Kitchen Converter</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                Accurate ingredient conversions for cooking and baking. 
                All conversion factors are based on standard ingredient densities.
              </p>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-3">Supported Units</h4>
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Grams (g)</li>
                <li>• Cups</li>
                <li>• Ounces (oz)</li>
                <li>• Teaspoons (tsp)</li>
                <li>• Tablespoons (tbsp)</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-gray-900 mb-3">Tips</h4>
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Spoon flour into cups for accuracy</li>
                <li>• Pack brown sugar measurements</li>
